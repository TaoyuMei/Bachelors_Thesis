...
my $EC_xls_error=$EC_xls."download_and_open_errors.txt";	#2018.6.28
...
while(<INFO>){	
	...
	my $toxin_name=$1;
	my $EC_number=$2;
	my $EC_url="http://www.kegg.jp/dbget-bin/www_bget?ec:".$EC_number;	 

	my $EC_status=getstore($EC_url,$EC_number);
	if(!is_success($EC_status)){	#if cannot download,wait 10 second and download again
		sleep(10);
		$EC_status=getstore($EC_url,$EC_number);
		}
	if(!is_success($EC_status)){	#if download fail again,print this EC number to error record, and skip this EC number
		print ERROR ("cannot download $EC_number\n");
		next;	#next <INFO> line
		}
...
#http://www.kegg.jp/dbget-bin/www_bget?-f+-n+a+eco:b0186  AA sequence
			my $dna_url="http://www.kegg.jp/dbget-bin/www_bget?-f+-n+n+".$gene_number;	
			my $aa_url="http://www.kegg.jp/dbget-bin/www_bget?-f+-n+a+".$gene_number;	
			$gene_number=~/(\w*):(\w*)/;	#file name cannot contain ":" in windows system
			$gene_number=$1.".".$2;
			my $dna=$gene_number."dna";
			my $aa=$gene_number."aa";

			$dna_status=getstore($dna_url,$dna);	#download the DNA sequence html
			if(!is_success($dna_status)){	#if cannot download, wait 10 seconds and try again
				sleep(10);
				$dna_status=getstore($dna_url,$dna);
				if(!is_success($dna_status)){	#if fail again,record the gene number in an error output file
					print ERROR ("cannot download $dna\n");
					}
				}
			if(is_success($dna_status)){	#if the download success anyhow,open the html file		
...
#KEGG gene number,KO number,enzyme name,EC number,gene name from RefSeq or GenBank or no gene name;			
			...
			$aa_status=getstore($aa_url,$aa);	#download the amino acid sequence html
			if(!is_success($aa_status)){	#if cannot download, wait 10 seconds and try again
				sleep(10);
				$aa_status=getstore($aa_url,$aa);
				if(!is_success($aa_status)){	#if download fail again, record the gene number
					print ERROR ("cannot download $aa\n");
					}
				}
			if(is_success($aa_status)){	#if the download success anyhow,open the html file				
